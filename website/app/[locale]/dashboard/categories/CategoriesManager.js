            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                {categories.map((category) => (
                    <div key={category.id} className="bg-white p-6 rounded-3xl shadow-sm border border-stone-100 hover:shadow-md transition-all">
                        ...
                        <h3 className="text-xl font-bold text-stone-900 mb-1">{category.name_ar}</h3>
                        <p className="text-stone-500 text-sm ltr">{category.name_en}</p>
                    </div>
                ))}
            </div>
                ...
                <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/50 backdrop-blur-sm">
                    <div className="bg-white rounded-3xl w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
                        ...
                        <form onSubmit={handleSubmit} className="p-6 space-y-4">
                            <div>
                                <label className="block text-sm font-medium text-stone-700 mb-1">اسم التصنيف (عربي)</label>
                                <input
                                    name="name_ar"
                            ...
                            <div>
                                <label className="block text-sm font-medium text-stone-700 mb-1">Category Name (English)</label>
                                <input
                                    name="name_en"
